...
import { deleteTechnicianById, createTechnician } from 'apiSdk/technicians';
...
function BusinessViewPage() {
  ...
  const technicianHandleCreate = async () => {
    setCreateError(null);
    try {
      await createTechnician({ business_id: id, user_id: technicianUserId });
  ...
  const technicianHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteTechnicianById(id);
      await mutate();
    } catch (error) {
      setDeleteError(error);
    }
            ...
            <Box>
              <Stack spacing={2} mb={8}>
                ...
                <Flex gap={5} alignItems="flex-end">
                  <Box flex={1}>
                    <UserSelect name={'technician_user'} value={technicianUserId} handleChange={setTechnicianUserId} />
                  </Box>
                  <Button
                    colorScheme="blue"
                    mt="4"
                    mr="4"
                    onClick={technicianHandleCreate}
                    isDisabled={!technicianUserId}
                  >
                    Create
                  </Button>
                </Flex>
                          ...
                          <Td>
                            {hasAccess('user', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                              <IconButton
                                onClick={(e) => {
                                  e.stopPropagation();
                                  technicianHandleDelete(record.id);
                                }}
                                colorScheme="red"
                                variant="outline"
                                aria-label="edit"
